...
This project implements a  web scraping agent using the AutoGen framework and an LLM.
...
- `config.py`  
  Loads environment variables from `.env` and defines `LLM_CONFIG` (model name, API key, and other parameters).
...
2.4. Configure API key (.env file)
Create a file named .env in the project root
MISTRAL_API_KEY=your_api_key_here

2.5 Run the code:

python main.py              # single example
python -m WebScraping.evaluate  # run all use-cases and save it in a file.
python -m WebScraping.multi_agent_evaluate # run all use-cases and save it in a file.
